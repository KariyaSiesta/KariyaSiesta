...
public class CheckWithProgress implements IRunnableWithProgress {
	...
	public void run(IProgressMonitor monitor) throws InvocationTargetException,
		...
		try {
			...
			throw new InterruptedException(
					Messages.getString("CheckWithProgress.1") + "\n" + message);
		}

		monitor.beginTask(
				Messages.getString("CheckWithProgress.0") + file.getName(),
				4 + rules.size());
		...
		try {
			...
			CheckerActivator.log(t);
			throw new InterruptedException(
					Messages.getString("CheckWithProgress.9") + getString(t));
		}
		...
		try {
			...
			String message = getString(t);
			throw new InterruptedException(
					Messages.getString("CheckWithProgress.11") + "\n" + message);
		}
		...
		if (monitor.isCanceled()) {
			throw new InterruptedException(
					Messages.getString("CheckWithProgress.14"));
		}
		monitor.worked(1);
	 ...
	 * @param severity
	 */
	public static IMarker createMarker(IFile file, int linenum, int start,
