...
/**
 * 式（Expr要素）がカッコ式かどうかを調べるXPath関数
 * @author uehara
 */
public class IsParenthetic implements XPathFunction {

	private static IsParenthetic instance;
	
	
	private IsParenthetic() {
	}
	
	
	public static IsParenthetic getInstance() {
		if (instance == null) {
			instance = new IsParenthetic();
		}
	...
	public Boolean evaluate(List args) throws XPathFunctionException {
		...
		return sort == CExpressionElement.Sort.PARENTHETIC;
	}
